This script calls SNPS from variants identified by samtools/bcftools or GATK (needs DP4 values though). It takes the following output:
...
for each isolate:
  for each set of 2 quality-30 vcf files:
...
For each isolate:
  Identify common positions between 2 aligners and store as a list
  ...
  For each equivalent object in the three filtered lists:
    If the DP4 values suggest a REF and agree among all 2 aligners:
      Add position:refbase to SNPs dictionary
    Elif the DP4 values suggest a SNP and agree among all 2 aligners:
      Add position:altbase to SNPs dictionary
    Else: # This is DP4 values 0.20 < x < 0.80, as well as cases where the three
          # aligners do not agree on a call (eg. bwa calls SNP, novo calls REF)
...
MUST be run on isolates with EXACTLY TWO (2) sets of vcf files.
