...
script, bwa30list, novo30list, reference_fasta, output_positions_file, bwa25list, novo25list, outputdir, outputfasta = sys.argv
...
# Identify hq positions from overlap of all 2 aligners at min qual 30:
for bwa, novo in itertools.izip(bwa30_list, novo30_list):
    common_positions = get_common_positions(bwa, novo)
...
# For every isolate:
for bwa, novo in itertools.izip(bwa25_list, novo25_list):

    # Identify common positions between 3 aligners:
    common_positions_25 = get_common_positions(bwa, novo)
    ...
    snps = {}
    for bwasnp, novosnp in itertools.izip(bwa_filtered_list, novo_filtered_list):
        snps[bwasnp.pos] = [bwasnp.ref, call_snps(bwasnp, novosnp)]
