            ...
            for item in temp_list3:
                if item.startswith("DP="):
                    if len(item) == 5:
                        ...
                        temp_list1.append(int(
                            temp_list3[temp_list3.index(item)][-3] + temp_list3[temp_list3.index(item)][-2] +
                            temp_list3[temp_list3.index(item)][-1]))
            for item in temp_list3:
...
def hq_pos_list(variantlist, indel_list):
    ...
    with open(totalhqposlist, 'a') as infile:
        for variant in variantlist:
            if variant.dp >= HQS_DEPTH and variant.quality >= HQS_QUALITY and variant.dp4[2] >= HQS_ALT_THRESHOLD and \
                            variant.dp4[3] >= HQS_ALT_THRESHOLD and (variant.dp4[0] + variant.dp4[1]) < HQS_REF_MAX * (
                variant.dp4[2] + variant.dp4[3]) and variant.pos > HQS_MIN_DIST and variant.pos < (
                HQS_REF_LENGTH - HQS_MIN_DIST) and float(variant.dp4[2]) / variant.dp4[3] > HQS_MIN_DP_PER and float(
                    variant.dp4[3]) / variant.dp4[2] > HQS_MIN_DP_PER and len(variant.ref) == 1 and len(
