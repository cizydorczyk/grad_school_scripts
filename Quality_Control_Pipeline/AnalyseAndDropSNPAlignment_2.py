...
script, inputfile, positions_file, outputtsv, outputfasta, newpositions = argv
...
with open(positions_file, 'r') as infile2:
    for line in infile2:
        if not line.startswith("Position"):
            ...
            positions_dict[line.strip().split('\t')[0]] = line.strip().split('\t')[1]
...
# Create new list of remaining positions and write to file:
remaining_pos = list(df1)

with open(newpositions, 'w') as outfile2:
    to_write2 = []
    for i in remaining_pos:
        print i, positions_dict[i]
        to_write2.append(str(i) + '\t' + positions_dict[i])
    outfile2.write('\n'.join(to_write2))
